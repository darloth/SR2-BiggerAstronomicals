...
class MakeBlackhole : MapHook {
	...
	Argument rad("Radius", AT_Range, "350.0", doc="Radius of the event horizon.");
...
class MakePlanet : MapHook {
	...
	double AVG_PLANET_HEALTH = 1200000000;
	...
	Argument radius(AT_Range, "12:48", doc="Size of the planet, can be a random range.");
	Argument orbit_spacing(AT_Range, "175:325", doc="Distance from the previous planet.");
...
#section server
	void trigger(SystemData@ data, SystemDesc@ system, Object@& current) const override {
		...
		double sizeFact = clamp(radius / 30.0, 0.1, 5.0);
